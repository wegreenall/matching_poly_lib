use crate::matching::graphs::Graph;
...
pub struct WeightedGraph{
    pub graph: Graph,
    pub weights: [u64; MAX_NODES*MAX_NODES],
}
...
impl WeightedGraph {
   ...
   pub fn from(data: [usize; mem::size_of::<usize>()*8], weights: [u64; MAX_NODES*MAX_NODES]) -> WeightedGraph {
        WeightedGraph {
...
pub fn _calculate_weighted_matching_polynomial_binary(weighted_graph: WeightedGraph) -> Polynomial<u64> {
    ...
    if weighted_graph.graph.edgeless() { // i.e. we're at the base case.
        ...
        let squared_weight = if let Some(x) = squared_weight { x } else { return Polynomial::new(vec![0]) };
